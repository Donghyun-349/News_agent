...
def main():
    ...
    for category in sorted_categories:
        ...
        for item in items:
            ...
            if is_korean_article(title, publisher):
                ko_items.append(item)
            else:
                en_items.append(item)
                
        # --- Step 1: Clustering (English) ---
        if en_items:
            logger.info(f"  -> Clustering {len(en_items)} English articles...")
            en_clusters = cluster_step1(model, category, en_items)
            for c in en_clusters:
                c["category"] = category
                c["lang"] = "EN"
            all_results.extend(en_clusters)

        # --- Step 1: Clustering (Korean) ---
        if ko_items:
            logger.info(f"  -> Clustering {len(ko_items)} Korean articles...")
            ko_clusters = cluster_step1(model, category, ko_items)
            for c in ko_clusters:
                c["category"] = category
                c["lang"] = "KO"
            all_results.extend(ko_clusters)
            
    # Custom Category Order Mapping
    CATEGORY_ORDER = {
        "G_mac": 1,
        "G_mak": 2,
        "G_tech": 3,
    ...
    # 정렬: 카테고리(지정 순서) -> 언어(EN 우선) -> 기사수 (Curated Count) (오름차순)
    # x.get("news_ids")의 길이가 curated_count입니다.
    # User feedback: Ensure G_mac is first, then specific order, then Language, then Count.
    all_results.sort(key=lambda x: (
        CATEGORY_ORDER.get(x.get("category"), 99), 
        0 if x.get("lang") == "EN" else 1, 
        len(x.get("news_ids", []))
    ))
    ...
    if not args.no_export:
        ...
        # Let's simple append all sorted by Category then Count (Language sorting is already applied above)
